...
if ($user->hasRight('categorie', 'creer')) {
	...
	if ($action == 'create' || $action == 'add') {
		...
		// print '</form>';
		$output .= "
		<form class='w-100' action='{$_SERVER['PHP_SELF']}'?type='{$type}' method='POST'>
		 ...
		 <input id='name' class='form-control mt-3' name='name' value='" . dol_escape_htmltag($name) . "'>
		 <div class='my-3'>
		 <input type='submit' class='button btn' value='Create Rack' name='creation' />
		 </div>
		</form> 
		</div>
		</div>
