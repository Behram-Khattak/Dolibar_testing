...
if (!isset($_GET['rowid'])) {
    // Retrieve and sanitize the 'container_id' value

    return "Id not found ";
}

$rowid = intval($_GET['rowid']);
// $sql = 'SELECT p.rowid, p.ref, p.container_id, p.description';
...
// $sql .= ' GROUP BY p.rowid, p.ref, p.container_id, p.description, p.label, p.fk_product_type, p.barcode, p.price, p.tva_tx, p.price_ttc, p.price_base_type, p.entity,';
// $sql .= ' p.duration, p.finished, p.tosell, p.tobuy, p.seuil_stock_alerte, p.desiredstock,';
// $sql .= ' p.tobatch, p.quantity, p.date_creation, p.tms, p.pmp, p.stock, p.cost_price,';
// $sql .= ' p.weight, p.weight_units, p.length, p.length_units, p.width, p.width_units, p.height, p.height_units, p.surface, p.surface_units, p.volume, p.volume_units,';
// $sql .= ' p.fk_country, p.fk_state, p.import_key';
...
$output = "<div class='row justify-content-center w-100'>
    <div class='col-md-12'>
        <div class='card'>
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_object()) {
        ...
        $output .= "<tr>
                        <td class='fw-bold'>Container ID:</td>
                    ...
                    <tr>
                        <td class='fw-bold'>Container Type:</td>
                        <td>" . $row->container_type . "</td>
                    </tr>
                    <tr>
                        <td class='fw-bold'>Container Arrival Date:</td>
